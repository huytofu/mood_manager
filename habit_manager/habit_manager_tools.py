...
class FinalHabitAnswerInput(BaseModel):
    ...
    # NEW: Advanced analytics fields from generate_habit_insights and other analytics tools
...
class FinalHabitAnswerOutput(BaseModel):
    ...
    # NEW: Advanced analytics fields to match generate_habit_insights and other analytics
...
def final_habit_answer(
    ...
    """
    Tool Purpose: Standardize final responses for habit management operations.
    This tool formats the final output that will be returned to the Master Manager.
    
    Must ALWAYS be called as the final step to provide standardized response format.
    Handles both basic operations (habit creation) and advanced analytics results.
    """
    
    # Process habit plan data
    habit_output = None
    if habit_plan:
        habit_output = HabitOutput(
            is_created=habit_plan.get("is_created", False),
            habit_id=habit_plan.get("habit_id"),
    ...
    # Combine analysis_result with individual analytics fields
    combined_analysis = {}
    if analysis_result:
        combined_analysis.update(analysis_result)
    
    # Add individual analytics results to combined analysis
    if underperforming_habits:
        combined_analysis["underperforming_habits"] = underperforming_habits
    if epic_progress_data:
        combined_analysis["epic_progress"] = epic_progress_data  
    if habit_interactions:
        combined_analysis["habit_interactions"] = habit_interactions
    if mood_correlations:
        combined_analysis["mood_correlations"] = mood_correlations
    if daily_plan:
        combined_analysis["daily_plan"] = daily_plan
    if progress_data:
        combined_analysis["progress_data"] = progress_data
    
    # Create the comprehensive output
    return FinalHabitAnswerOutput(
        habit_plan=habit_output,
        analysis=combined_analysis if combined_analysis else None,
        recommendations=recommendations,
        intervention_type=intervention_type,
        error_type=error_type,
        # Pass through all analytics fields
        insights=insights,
        patterns=patterns,
...
async def generate_habit_insights(
    ...
    This function integrates results from other analysis functions to provide unified insights.
    
    Args:
    - user_id (str): User identifier
    - habit_id (Optional[str]): Specific habit ID, or None for all habits
    - insight_type (str): Type of insights (completion_patterns, timing_optimization, comprehensive)
    - underperformance_analysis (Optional[Dict]): Results from analyze_underperforming_habits
    - epic_progress_analysis (Optional[Dict]): Results from analyze_lagging_epic_progress
    - interaction_analysis (Optional[Dict]): Results from analyze_habit_interactions
    - mood_correlation_analysis (Optional[Dict]): Results from analyze_mood_habit_correlation
    ...
    # INTEGRATE ANALYSIS RESULTS FROM OTHER FUNCTIONS
    if underperformance_analysis and underperformance_analysis.get("success"):
        integrated_analysis["underperformance"] = underperformance_analysis
        
        # Extract insights from underperformance analysis
        underperforming = underperformance_analysis.get("underperforming_habits", [])
        if underperforming:
            insights.append(f"UNDERPERFORMANCE DETECTED: {len(underperforming)} habits below threshold")
            
            # Add specific insights for top underperforming habits
            for habit in underperforming[:2]:  # Top 2
                habit_type = habit.get("habit_type", "formation")
                completion_rate = habit.get("completion_rate", 0)
                
                if habit_type == "breaking":
                    insights.append(f"High relapse rate for '{habit['habit_name']}': {(1-completion_rate)*100:.1f}% relapse frequency")
                    recommendations.append(f"PRIORITY: Strengthen relapse prevention for '{habit['habit_name']}'")
                else:
                    insights.append(f"Low completion for '{habit['habit_name']}': {completion_rate*100:.1f}% success rate")
                    recommendations.append(f"PRIORITY: Simplify or reschedule '{habit['habit_name']}'")
    
    if epic_progress_analysis and epic_progress_analysis.get("success"):
        integrated_analysis["epic_progress"] = epic_progress_analysis
        
        # Extract insights from epic progress analysis
        epic_data = epic_progress_analysis.get("epic_analysis", {})
        progress_gap = epic_data.get("progress_gap", 0)
        
        if progress_gap > 10:
            insights.append(f"EPIC GOAL LAGGING: {progress_gap:.1f}% behind expected progress")
            
            bottlenecks = epic_progress_analysis.get("bottleneck_habits", [])
            if bottlenecks:
                worst_bottleneck = bottlenecks[0]
                insights.append(f"Primary bottleneck: '{worst_bottleneck['habit_name']}' at {worst_bottleneck['consistency_rate']*100:.1f}% consistency")
                recommendations.append(f"URGENT: Focus all effort on improving '{worst_bottleneck['habit_name']}' consistency")
    
    if interaction_analysis and interaction_analysis.get("success"):
        integrated_analysis["interactions"] = interaction_analysis
        
        # Extract insights from interaction analysis
        antagonistic = interaction_analysis.get("antagonistic_pairs", [])
        synchronous = interaction_analysis.get("synchronous_pairs", [])
        
        if antagonistic:
            insights.append(f"HABIT CONFLICTS: {len(antagonistic)} antagonistic relationships detected")
            top_conflict = antagonistic[0]
            insights.append(f"Strongest conflict: '{top_conflict['habit1_name']}' vs '{top_conflict['habit2_name']}'")
            recommendations.append(f"RESOLVE CONFLICT: Separate timing for '{top_conflict['habit1_name']}' and '{top_conflict['habit2_name']}'")
        
        if synchronous:
            insights.append(f"POSITIVE SYNERGIES: {len(synchronous)} supportive habit relationships found")
            top_synergy = synchronous[0]
            insights.append(f"Strongest synergy: '{top_synergy['habit1_name']}' + '{top_synergy['habit2_name']}'")
            recommendations.append(f"LEVERAGE SYNERGY: Schedule '{top_synergy['habit1_name']}' and '{top_synergy['habit2_name']}' together")
    
    if mood_correlation_analysis and mood_correlation_analysis.get("success"):
        integrated_analysis["mood_correlations"] = mood_correlation_analysis
        
        # Extract insights from mood correlation analysis
        correlations = mood_correlation_analysis.get("correlations", {})
        
        for habit_id_key, correlation_data in correlations.items():
            mood_score_corr = correlation_data.get("mood_score_correlation", 0)
            crisis_impact = correlation_data.get("crisis_impact", 0)
            
            if abs(mood_score_corr) > 0.3:
                direction = "positively" if mood_score_corr > 0 else "negatively"
                insights.append(f"MOOD CORRELATION: Habit performance {direction} correlated with mood")
                
            if crisis_impact < -0.4:
                insights.append(f"CRISIS VULNERABILITY: Habit significantly impacted during crisis periods")
                recommendations.append("CRISIS PLANNING: Develop crisis-adaptive versions of vulnerable habits")
    ...
    if habit_id:
        ...
        if completion_records:
            # Completion patterns (only if not already analyzed)
            if insight_type in ["completion_patterns", "comprehensive"] and not underperformance_analysis:
            ...
            # Timing optimization (only if not already analyzed)
            if insight_type in ["timing_optimization", "comprehensive"] and not interaction_analysis:
        ...
        # Multi-habit analysis (only if not already provided by other analyses)
        if not underperformance_analysis and not epic_progress_analysis:
    ...
    if len(integrated_analysis) > 1:
        insights.append("MULTI-FACTOR ANALYSIS: Multiple habit challenges detected - prioritizing interventions")
        
        # Cross-analysis recommendations
        if "underperformance" in integrated_analysis and "interactions" in integrated_analysis:
            recommendations.append("STRATEGY: Address habit conflicts first, then focus on underperforming habits")
        
        if "epic_progress" in integrated_analysis and "mood_correlations" in integrated_analysis:
            recommendations.append("HOLISTIC APPROACH: Epic goal progress affected by mood - integrate mood management")
    
    # Ensure we have some insights even if no external analysis was provided
    if not insights:
        insights.append("Analysis complete - limited data available for deeper insights")
        recommendations.append("Continue tracking habits for more comprehensive analysis")
